...
class perusahaanServices
{
    ...
    public function handleFormPerusahaan(Request $request)
    {
        ...
        try {
            ...
            DataFinance::updateOrCreate(
                ['identitas_perusahaan_id' => $data->id],
                [
                    'nama' => strtoupper($request->nama_finance),
                    'no_hp' => $request->no_hp_finance,
                    'email' => $request->email_finance,
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]
            );
            ...
            if ($request->bentuk_usaha == 'perseorangan') {
                ...
                if ($request->hasil_ttd) {
                    ...
                    UploadTTD::handleUpload(($oldData ? ($oldData->data_identitas ? ((string) $oldData->data_identitas['ttd'] ?: '') : '') : ''), $path, $imageName, $data->id);
                } else {
                ...
                if ($request->status_pkp == 'pkp') {
                    if ($request->hasFile('foto_sppkp')) {
                        ...
                        $filename = uniqid() . '-SPPKP-' . Str::slug($request->nama_npwp, '-') . '.' . strtolower($foto->getClientOriginalExtension());
                        ...
                        DB::table('identitas_perusahaan')->where('id', $data->id)->update([
                            'sppkp' => $oldData->sppkp,
                            'status_upload_sppkp' => $oldData->status_upload_sppkp
                        ]);
                    }
                }
            }
            ...
            if ($request->hasFile('foto_penanggung')) {
                ...
                $filename = uniqid() . '-PIC-' . strtoupper($request->identitas_penanggung_jawab) . '-' . Str::slug($request->nama_penanggung_jawab, '-') . '.' . strtolower($foto->getClientOriginalExtension());
                ...
                UploadIdentitas::handleUpload($filename, ($oldData ? ($oldData->data_identitas ? ($oldData->data_identitas->foto ?: '') : '') : ''), $tempPath, $data->id);
            } else {
                DB::table('data_identitas')->where('identitas_perusahaan_id', $data->id)->update([
                    'foto' => $oldData->data_identitas->foto,
                    'status_upload_foto' => $oldData->data_identitas->status_upload_foto
                ]);
            }
