...
class base30ToImage
{
    ...
    // // Karakter set base30
    // private $base30characters = "0123456789abcdefghijklmnopqrstuvwxyz";

    // // Fungsi untuk mendecode data base30 ke array koordinat asli
    // public function base30ToNative($base30)
    // {
    //     $raw = array();
    //     $strokes = explode(" ", $base30);
        
    //     foreach ($strokes as $stroke) {
    //         $coords = explode("_", $stroke);
    //         $x_coords = $this->decodePoints($coords[0]);
    //         $y_coords = $this->decodePoints($coords[1]);
    //         $raw[] = ['x' => $x_coords, 'y' => $y_coords];
    //     }

    //     return $raw;
    // }

    // // Fungsi untuk mendecode point base30 menjadi angka asli
    // private function decodePoints($points)
    // {
    //     $coords = [];
    //     $lastPoint = 0;

    //     for ($i = 0; $i < strlen($points); $i++) {
    //         $point = strpos($this->base30characters, $points[$i]);
    //         if ($point === false) continue;

    //         // Base30 delta encoding
    //         $lastPoint += ($point - 15);
    //         $coords[] = $lastPoint;
    //     }
