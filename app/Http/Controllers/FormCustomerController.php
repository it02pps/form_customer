...
class FormCustomerController extends Controller
{
    ...
    public function upload_pdf(Request $request, $menu, $id)
    {
		...
		if($oldData) {
			$responses = Http::pool(function ($pool) use ($services, $oldData) {
				$requests = [];

        			foreach($services as $service) {
					$requests[] = $pool->timeout(10)->withHeaders([
			...
			foreach($services as $i => $service) {
				try {
					if(isset($responses[$i]) && $responses[$i]->ok()) {
                ...
                $resUpload = Http::timeout(30)->withHeaders([
                    'x-api-key' => config('services.service_v.api_key'),
                ])->attach(
                    'file',
